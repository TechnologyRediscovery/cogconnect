...
def main():
    ...
    global PARID_FILE
    PARID_FILE = 'parcelidlists/assessments.csv'
...
def get_nextparcelid(input_file):
    ...
    with open(input_file, 'r', encoding=CSV_FILE_ENCODING) as csv_file:
        ...
        for row in reader:
            paridmap = dict(parid=str(row['PARID']).strip(), municode=str(row['MUNICODE'].strip()))
            yield paridmap
...
def create_and_insert_unitzero(propertyid):
    ...
    """INSERT INTO public.propertyunit(
	unitid, unitnumber, property_propertyid, otherknownaddress, notes,
    rentalintentdatestart, rentalintentdatestop, rentalintentlastupdatedby_userid, rentalnotes, active, condition_intensityclassid, lastupdatedts)
	VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?);"""
    insertmap = {}
...
def get_county_page_for(parcel_id):
    ...
    waittime = random.uniform(0.0,0.3)
...
county_info_tax_cache = {}
def get_county_taxpage_for(parcel_id):
    if parcel_id in county_info_tax_cache:
        return county_info_cache[parcel_id]
    COUNTY_REAL_ESTATE_URL = ('http://www2.county.allegheny.pa.us/'
                              'RealEstate/Tax.aspx?')
    search_parameters = {
        ...
        'SearchType': 3,
        'SearchParcel': parcel_id}
    waittime = random.uniform(0.0,0.1)
    print("waiting:" + str(waittime))
    time.sleep(waittime)
    try:
        response = requests.get(
        ...
        response = requests.get(
                ...
                params=search_parameters,
                timeout=5)
    county_info_tax_cache[parcel_id] = response.text
    return response.text
...
def extract_and_insert_person(rawhtml, propertyid, propinserts):
    ...
    insertmap['muni_municode'] = propinserts['municode']
    ...
    try:
        ...
        print('extracted owner raw:' + insertmap['rawhtml'] )
...
def create_ce_event(caseid):
    ...
    requiresviewconfirmation, hidden, notes, requestedevent_eventid, directrequesttodefaultmuniceo)
    VALUES (DEFAULT, 211, %(caseid)s, now(), now(),
    'Update event from county', 99, TRUE, FALSE, FALSE,
...
def get_propid(parid):
    db_conn = get_db_conn()
    cursor = db_conn.cursor()
    
    propidsearchsql = """SELECT propertyid FROM public.property WHERE parid=%(par)s"""
    cursor.execute(propidsearchsql, dict(par=parid))
    propid = cursor.fetchall()
    return propid[0][0]

def get_personid():
    db_connn = get_db_conn()
    cursor = db_conn.cursor()
    
    personidsql = "SELECT MAX(personid) FROM public.person"
    cursor.execute(personidsql)
    personid = cursor.fetchall()[0][0]
    print("Person ID: " + str(personid))
    return personid

def check_tax_status(rawhtmltax):
    TAX_STATUS_SPAN_ID = 'lblTaxInfo'
    rows = 0
    tax_flag = False
    
    soup = bs4.BeautifulSoup(rawhtmltax, 'lxml')
    
    raw_info = soup.find('span', id=TAX_STATUS_SPAN_ID).text
    scrapedhtml = soup.select('#lblTaxInfo > table > tr')
    
    
    for row in range(1, len(scrapedhtml)):
        soup = bs4.BeautifulSoup(str(scrapedhtml[row]), 'lxml')
        col = soup.select('tr > td')
        paystatus = col[1].text
        if paystatus != 'PAID':
            tax_flag = True
            return tax_flag    
    return tax_flag    
...
def get_tax_address(rawhtml):
    TAX_ADDRESS_SPAN_ID = 'lblMortgage'
    
    soup = bs4.BeautifulSoup(rawhtml, 'lxml')
    
    info = soup.find('span', id = TAX_ADDRESS_SPAN_ID)
    return info.get_text(" ", strip=True)
    


def insert_property_external_data(propid, data):
    db_conn = get_db_conn()
    cursor = db_conn.cursor()
    
    extsql = """INSERT INTO public.propertyexternaldata(
    extdataid, property_propertyid, ownername, ownerphone, address_street,
    address_citystatezip, address_city, address_state, address_zip, saleprice,
    saleyear, assessedlandvalue, assessedbuildingvalue, assessmentyear, usecode,
    yearbuilt, livingarea, condition, taxstatus, taxstatusyear, notes, lastupdated)
    VALUES (DEFAULT, %(propid)s, NULL, NULL, %(taxaddress)s,
    NULL, NULL, NULL, NULL, NULL,
    NULL, NULL, NULL, NULL, NULL,
    NULL, NULL, NULL, %(taxstatus)s, NULL, NULL, now());"""
    
    extmap = {}
    extmap['propid'] = propid
    extmap['taxstatus'] = data['taxstatus']
    extmap['taxaddress'] = data['taxaddress']
    cursor.execute(extsql, extmap)
    db_conn.commit()
    print("property external data successfully entered")
...
def insert_property_basetableinfo():
    ...
    insert_sql = """
        ...
        VALUES (DEFAULT, %(muni)s, %(parcelid)s, %(addr)s, 
    ...
    """INSERT INTO public.property(
            ...
            lastupdated, lastupdatedby, locationdescription, multiunit,
            ...
            landbankprospect_intensityclassid, landbankheld, active, nonaddressable, usetype_typeid)
	VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?);"""
    insertmap = {}
    propertycount = 0
    personcount = 0
    # the main delegator loop
    for parcel in parcelgenerator:
        # go get raw HTML
        parid = parcel['parid']
        municode = parcel['municode']
        ...
        # and sql errors bubbling up from the extraction methods that also commit
            ...
            #print(1)
            extract_and_insert_person(rawhtml, propid, addrmap)
            #print(2)
        ...
        try:
            ...
            break
        try:
            taxhtml = get_county_taxpage_for(parid)
            taxstatus = check_tax_status(taxhtml)
            externaldata = {'taxstatus':None}
            if taxstatus:
                externaldata['taxstatus'] = 'Possible Unpaid Taxes'
            externaldata['taxaddress'] = get_tax_address(taxhtml)
            insert_property_external_data(propid, externaldata)
            print("Taxes successfully scraped")
        except:
            print("Error scraping or inserting tax info")
            tracebackck.print_exc()
